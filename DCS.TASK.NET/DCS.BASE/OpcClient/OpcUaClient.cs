...
namespace OpcUaHelper
{
    ...
    public class OpcUaClient
    {
        ...
        {
            get { return m_IsConnected && m_session != null; }
        }
        ...
        {
            List<DataValue> readResult = null;
            ...
            if (m_session != null)
            {
                ...
                readResult = results.ToList();
            }
            else
            {
                readResult = new List<DataValue>();
            }
        ...
        public bool WriteNodes(string[] tags, object[] values, ushort namespaceIndex)
        {
            bool result = false;
            ...
            if (m_session != null)
            {
                ...
                for (int i = 0; i < tags.Length; i++)
                {
                    if (i < values.Length)
                    {
                        WriteValue valueToWrite = new WriteValue()
                        {
                            NodeId = new NodeId(tags[i], namespaceIndex),
                            AttributeId = Attributes.Value,
                        };
                        ...
                        //valueToWrite.Handle = attributeIdToString(Attributes.Value);
        ...
        Hashtable m_hashAttributeNames;
