...
namespace DCS.BASE
{
    ...
    public class RequestToHttpHelper
    {
        ...
        public async Task<HttpResponseResultModel<T>> PostAsync<T>(HttpRequestModel requestModel)
        {
            ...
            HttpResponseResultModel <T> httpResponseResultModel = new HttpResponseResultModel<T>();

            try
            {
                HttpResponseMessage response = await client.SendAsync(request, cancelSource.Token).ConfigureAwait(false);

                httpResponseResultModel.HttpStatusCode = response.StatusCode;
                httpResponseResultModel.IsSuccess = response.IsSuccessStatusCode;

                var content = await response.Content.ReadAsStringAsync().ConfigureAwait(false);

                httpResponseResultModel.BackResult = JsonHelper.DeserializeObject<T>(content);
            }
            catch (Exception ex)
            { }
            finally
