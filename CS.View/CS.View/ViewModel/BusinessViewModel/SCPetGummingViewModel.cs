...
{
    internal class SCPetGummingViewModel : DataProcess<SCPetGummingEntity>
    {
        /// <summary>
        /// 参数管理名管理接口
        /// </summary>
        readonly ISCPetGummingService _petService;

        /// <summary>
        /// 参数明细
        /// </summary>
        readonly SCPetGummingEntity petEntity;

        public SCPetGummingViewModel(ISCPetGummingService petService)
        {
        ...
        /// 参数明细命令
        /// </summary>
        public RelayCommand<SCPetGummingEntity> ParamDetailCommand { get; private set; }
        /// <summary>
        /// 清空编辑框
        /// </summary>
        ...
        {
            //查询数据
            var result = await _petService.GetSCPetGummingAsync(pageInex, PageSize, new SCPetGummingCondition()
            {
        ...
        protected async override void Save()
        {
            ...
            if (Mode == ActionMode.Add)
            {
                ...
                //添加数据
                result = await _petService.AddSCPetGummingAsync(Model);
            }
            else if (Mode == ActionMode.Edit)
            {
                result = await _petService.UpdateSCPetGummingAsync(Model.Id, Model);
            }
        ...
        protected async override void Delete(SCPetGummingEntity model)
        {
            bool result = await _petService.DeteleSCPetGummingAsync(model.Id);
