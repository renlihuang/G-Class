...
{
    internal class SCPetAssembleViewModel : DataProcess<SCPetAssembleEntity>
    {
        /// <summary>
        /// 参数管理名管理接口
        /// </summary>
        readonly ISCPetAssembleService _petService;

        /// <summary>
        /// 参数明细
        /// </summary>
        readonly SCPetAssembleEntity petEntity;

        public SCPetAssembleViewModel(ISCPetAssembleService petService)
        {
        ...
        /// 参数明细命令
        /// </summary>
        public RelayCommand<SCPetAssembleEntity> ParamDetailCommand { get; private set; }
        /// <summary>
        /// 清空编辑框
        /// </summary>
        ...
        {
            //查询数据
            var result = await _petService.GetSCPetAssembleAsync(pageInex, PageSize, new SCPetAssembleCondition()
            {
        ...
        protected async override void Save()
        {
            ...
            if (Mode == ActionMode.Add)
            {
                ...
                //添加数据
                result = await _petService.AddSCPetAssembleAsync(Model);
            }
            else if (Mode == ActionMode.Edit)
            {
                result = await _petService.UpdateSCPetAssembleAsync(Model.ID, Model);
            }
        ...
        protected async override void Delete(SCPetAssembleEntity model)
        {
            bool result = await _petService.DeteleSCPetAssembleAsync(model.ID);
