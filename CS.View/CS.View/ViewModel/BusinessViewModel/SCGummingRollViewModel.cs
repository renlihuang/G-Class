...
{
    internal class SCGummingRollViewModel : DataProcess<SCGummingRollEntity>
    {
        /// <summary>
        /// 参数管理名管理接口
        /// </summary>
        readonly ISCGummingRollService _petService;

        /// <summary>
        /// 参数明细
        /// </summary>
        readonly SCGummingRollEntity petEntity;

        public SCGummingRollViewModel(ISCGummingRollService petService)
        {
        ...
        /// 参数明细命令
        /// </summary>
        public RelayCommand<SCGummingRollEntity> ParamDetailCommand { get; private set; }
        /// <summary>
        /// 清空编辑框
        /// </summary>
        ...
        {
            //查询数据
            var result = await _petService.GetSCGummingRollAsync(pageInex, PageSize, new SCGummingRollCondition()
            {
        ...
        protected async override void Save()
        {
            ...
            if (Mode == ActionMode.Add)
            {
                ...
                //添加数据
                result = await _petService.AddSCGummingRollAsync(Model);
            }
            else if (Mode == ActionMode.Edit)
            {
                result = await _petService.UpdateSCGummingRollAsync(Model.ID, Model);
            }
        ...
        protected async override void Delete(SCGummingRollEntity model)
        {
            bool result = await _petService.DeteleSCGummingRollAsync(model.ID);
