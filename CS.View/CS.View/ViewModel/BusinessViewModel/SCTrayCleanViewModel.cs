...
{
    internal class SCTrayCleanViewModel : DataProcess<SCTrayCleanEntity>
    {
        /// <summary>
        /// 参数管理名管理接口
        /// </summary>
        readonly ISCTrayCleanService _petService;

        /// <summary>
        /// 参数明细
        /// </summary>
        readonly SCTrayCleanEntity petEntity;

        public SCTrayCleanViewModel(ISCTrayCleanService petService)
        {
        ...
        /// 参数明细命令
        /// </summary>
        public RelayCommand<SCTrayCleanEntity> ParamDetailCommand { get; private set; }
        /// <summary>
        /// 清空编辑框
        /// </summary>
        ...
        {
            //查询数据
            var result = await _petService.GetSCTrayCleanAsync(pageInex, PageSize, new SCTrayCleanCondition()
            {
        ...
        protected async override void Save()
        {
            ...
            if (Mode == ActionMode.Add)
            {
                ...
                //添加数据
                result = await _petService.AddSCTrayCleanAsync(Model);
            }
            else if (Mode == ActionMode.Edit)
            {
                result = await _petService.UpdateSCTrayCleanAsync(Model.ID, Model);
            }
        ...
        protected async override void Delete(SCTrayCleanEntity model)
        {
            bool result = await _petService.DeteleSCTrayCleanAsync(model.ID);
