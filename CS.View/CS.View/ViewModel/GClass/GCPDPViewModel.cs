...
{
    internal class GCPDPViewModel : DataProcess<GCPDPEntity>
    {
        /// <summary>
        /// 参数管理名管理接口
        /// </summary>
        private readonly IGCPDPService _petService;
        ...
        public GCPDPViewModel(IGCPDPService petService)
        {
        ...
        /// 参数明细命令
        /// </summary>
        public RelayCommand<GCPDPEntity> ParamDetailCommand { get; private set; }
        ...
        {
            //查询数据
            var result = await _petService.GetGCPDPAsync(pageInex, PageSize, new GCPDPCondition()
            {
        ...
        protected override async void Save()
        {
            ...
            if (Mode == ActionMode.Add)
            {
                ...
                //添加数据
                result = await _petService.AddGCPDPAAsync(Model);
            }
            else if (Mode == ActionMode.Edit)
            {
                result = await _petService.UpdateGCPDPAAsync(Model.Id, Model);
            }
        ...
        protected override async void Delete(GCPDPEntity model)
        {
            bool result = await _petService.DeteleGCPDPAAsync(model.Id);
