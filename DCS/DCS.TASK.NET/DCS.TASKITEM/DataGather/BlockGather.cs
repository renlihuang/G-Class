        ...
        public void DoTask()
        {
            ...
            if (opc.ReadNodes(data))
            {
                ...
                if (PlcFalg == 1 && isFinish)
                {
                    ...
                    if (code == 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK释放模组号接口调用成功", "");
                    }
                    else if (code > 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK释放模组号接口调用失败", "");
                        _collectTaskContext.TaskMsgOperator.SetText("CODE: " + code);
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK释放模组号返回代码：", code.ToString());
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK释放模组号返回信息", msg);
                    ...
                    if (code == 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK电芯校验接口调用成功", "");
                    }
                    else if (code > 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK电芯校验接口调用失败", "");
                        _collectTaskContext.TaskMsgOperator.SetText("CODE: " + code);
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK电芯校验返回代码：", code.ToString());
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK电芯校验返回信息", msg);
                        //_outStaionResult = false;
                    }
                    else if (code == -1)
                    {
                    ...
                    if (code == 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK模组装配接口调用成功", "");
                    }
                    else if (code > 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK模组装配接口调用失败", "");
                        _collectTaskContext.TaskMsgOperator.SetText("CODE: " + code);
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK模组装配返回代码：", code.ToString());
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK模组装配返回信息", msg);
                        //_outStaionResult = false;
                    }
                    else if (code == -1)
                    {
                    ...
                    if (code == 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK数据收集接口调用成功", "");
                    }
                    else if (code > 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK数据收集接口调用失败", "");
                        _collectTaskContext.TaskMsgOperator.SetText("CODE: " + code);
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK数据收集返回代码：", code.ToString());
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK数据收集返回信息", msg);
                        //_outStaionResult = false;
                    }
                    else if (code == -1)
                    {
                    ...
                    if (code == 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK进站接口口调用成功", "");
                    }
                    else if (code > 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK进站接口调用失败", "");
                        _collectTaskContext.TaskMsgOperator.SetText("CODE: " + code);
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK进站返回代码：", code.ToString());
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK进站返回信息", msg);
                        //_outStaionResult = false;
                    }
                    else if (code == -1)
                    {
