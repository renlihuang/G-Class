        ...
        public void DoTask()
        {
            ...
            if (opc.ReadNodes(data))
            {
                ...
                if (PlcFalg == 1 && isFinish)
                {
                    ...
                    try
                    {
                        MESwebservice.AutoWeight.dataCollectForSfcExResponse sfcResponse = BlockCall.SeekSiteDatacoll(@AppConfig.WebserviceiniPath, "Block入packing收数", "");
                        code = sfcResponse.@return.code;
                        msg = sfcResponse.@return.message;
                    }
                    catch (Exception ex)
                    ...
                    if (code == 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK入箱收数出站接口调用成功", "");
                    }
                    else if (code > 0)
                    {
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK入箱收数出站接口调用失败", "");
                        _collectTaskContext.TaskMsgOperator.SetText("CODE: " + code);
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK入箱收数出站返回代码：", code.ToString());
                        _collectTaskContext.TaskMsgOperator.SetPairText("BLOCK入箱收数出站返回信息", msg);
